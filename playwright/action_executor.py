        ...
        for selector in selectors:
            try:
                if self.page.locator(selector).count() > 0:
                    ...
                    element.wait_for(state="visible", timeout=2000)
                    element.fill(value)
                    print(" Filled field successfully")
                    return True
            except Exception:
                continue
...
USAGE:
  ...
  # List all buttons on search results page
  python action_executor.py --url "https://google.com/search?q=python" --list-buttons
  
  # Fill form and submit
  python action_executor.py --url "https://example.com/form" --fill "email" "test@test.com" --click "Submit"
...
Note: This script integrates with EXISTING Google Search capabilities.
    ...
    parser.add_argument(
        "--url", type=str, required=True, help="Current page url from search results"
    )
    parser.add_argument("--click", type=str, help="Button text to click")
    parser.add_argument(
        "--click-exact", type=str, help="Button text to click (exact match)"
    )
    parser.add_argument(
        "--fill", nargs=2, metavar=("FIELD", "VALUE"), help="Fill input field"
    )
    parser.add_argument(
        "--list-buttons", action="store_true", help="List all buttons on page"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Seconds to wait before closing (default: 2)",
    )
