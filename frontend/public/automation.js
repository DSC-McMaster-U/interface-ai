...
(function () {
  ...
  // ── Screenshot (async — captured by background.js, saved to disk) ──

  /**
   * Requests a screenshot from background.js.
   *
   * params.savePath — (optional) folder to save into, e.g. "C:/screenshots"
   *                   Falls back to the browser's default download directory.
   * params.filename — (optional) custom filename, e.g. "myshot.png"
   *                   Falls back to "interfaceai_<timestamp>.png"
   *
   * background.js handles the actual capture + chrome.downloads.download().
   */
  function requestScreenshot(params, callback) {
  ...
  function connectWS() {
    try {
      ...
      ws.onmessage = function (event) {
        ...
        // Sends back base64 PNG so the CLI server can save it
        // to the extension folder (or anywhere on disk).
        if (msg.action === 'screenshot') {
          requestScreenshot(msg.params || {}, function (result) {
            // result.dataUrl is "data:image/png;base64,..."
            // CLI server should strip the prefix and fs.writeFileSync the buffer.
            ws.send(JSON.stringify({
              type: 'result',
              id: msg.id,
              result: {
                ...
                // raw base64 PNG (no data: prefix) for easy fs.writeFile on CLI side
                base64: result.dataUrl ? result.dataUrl.replace(/^data:image\/png;base64,/, '') : null,
              }
            }));
          });
          return;
