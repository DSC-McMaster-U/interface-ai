...
(function () {
  ...
  function scrollUp(pixels) {
    ...
    return { success: true, scrolledBy: -(pixels || 500) };
  }
  ...
  function typeText(text) {
    ...
    if (!active || active === document.body)
      return { success: false, error: "No focused input" };
  ...
  function getPageStatus() {
    return {
      ...
      headings: [].slice
        .call(document.querySelectorAll("h1,h2,h3"), 0, 15)
        .map(function (h) {
          return {
            level: h.tagName,
      ...
      links: [].slice
        .call(document.querySelectorAll("a[href]"), 0, 20)
        .map(function (a) {
          return {
    ...
    try {
      switch (action) {
        case "clickAtCoordinate":
          return clickAtCoordinate(params.x, params.y);
        case "clickByName":
          return clickByName(params.name, params.exactMatch);
        case "scrollUp":
          return scrollUp(params.pixels);
        case "scrollDown":
          return scrollDown(params.pixels);
        case "scrollToTop":
          return scrollToTop();
        case "scrollToBottom":
          return scrollToBottom();
        case "fillInput":
          return fillInput(params.identifier, params.value);
      ...
      ws.onmessage = function (event) {
        ...
        if (msg.action === "screenshot") {
          requestScreenshot(function (result) {
            ws.send(
              JSON.stringify({ type: "result", id: msg.id, result: result }),
            );
          });
          return;
        }
